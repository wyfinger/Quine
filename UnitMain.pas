...
begin
 ...
 for r := 0 to IM.RowCount-1 do
   begin
     s := 0;
     c2 := 0;
     for c := 0 to IM.ColCount-1 do
       if IM[r,c] <> ' ' then
         begin
           Inc(s);
           c2 := c;
     ...
     if s = 1 then
       begin
         for r2 := 0 to IM.RowCount-1 do
           if IM[r2,c2] = 'x' then IM[r2,c2] := 'X';
         IM[r,c2] := '*';
       end;
   end;
 MatrixToGrid(IM, grdImplets, 'V', 'I');
...
begin
 ...
 if grdInput.Row-1 = Inp.RowCount-1 then
   Inp.AddRow(' ')
 else
   Inp.InsertRow(grdInput.Row-1, ' ');
 MatrixToGrid(Inp, grdInput, 'V');
end;
